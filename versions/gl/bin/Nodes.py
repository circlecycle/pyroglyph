...
import sys, time, threading, re, imp, weakref, copy, pdb
...
####### load global basepath attributes before anything local runs #################### # # #  #   #    #

#current dir is the application package..
baselibs = 'lib'
...
class Node:
  """
    ...
    Usage (new canvas):   Nodes.Node(canvasElement)
          (new children): Nodes.Node(nodeElem, parent=parent)
  """  
  ...
  def __init__(self, elem, parent=False, after=False, attrs={}, topmostnode=True):
    ...
    self.__dict__['_displaytype'] = 'node'
    self.__dict__['_elem'] = elem
    self.__dict__['_classname'] = elem.tagName
    self.__dict__['_after'] = after
    ...
    #self.__dict__['node_text'] = u''
    
    ### attempt to attach the node text, this is useful when trying to create a button class ###
    ### <node>This</node> --> node['node_text'] -> This ###
    ### -Ken ###
    ### And hence later removed. ###
    
    #try:
    #  self.__dict__['node_text'] = elem.childNodes[0].nodeValue
    ...
    ####### if this node represents something other then a base tag (defined in Basetags) #################### # # #  #   #    #
    ####### copy over the dom nodes that the inherited classes provide.                   #################### # # #  #   #    #

    if CLASSES_ACCUMULATOR.has_key(self._classname):
      self.__buildclass(elem, CLASSES_ACCUMULATOR[self._classname])
    ...
    elif self._classname not in self.CLASS_DECORATOR_NODES:           
    ...
    if self._classname not in self.USES_INNER_XML_TYPE:
  ...
  def __initNameSpaceForCanvas(self):
    ...
    [GLOBALS_ACCUMULATOR.append( ( x.strip(), _import(x.strip()) ) ) 
      for x in self._elem.getAttribute('import').split(',') 
        if x]
    ...
    self.applydefaults( {'title': "PyroGlyph Application", 'framerate': 30.0, 'maxsize':"",
                         ...
                         'init':False, 'focused':self, 'lastFocused':None, '_uidcount':0, 'sizing':'off' } )
  ...
  def __initNameSpaceForNode(self):
    ...
    if self._after and self._after in self.parent.childNodes:
      self.parent.childNodes.insert(self.parent.childNodes.index(self._after), self)
    else:
    ...
    if self._elem.hasAttribute('name'):  
      self.parent.__dict__[self._elem.getAttribute('name')] = self
      
    #if the child has an id place a reference to it under the canvas #################### # # #  #   #    #
    if self._elem.hasAttribute('id'):  
      canvas.__dict__[self._elem.getAttribute('id')] = self
  ...
  def constrain(self, attr=None, func=None, set=False):
    """ make a constraint, that is, add to _handlers on a target node a pointer
  ...
  def create(self, elem, parent=None, after=False, attrs={}, topmostnode=True, msg=False):
    ...
    try:    func = getattr(Basetags, tagname)
  ...
  def delete(self):
    ...
    #self.parent._elem.removeChild(self._elem)
  ...
  def touch(self, attr):
    ...
    #distribute this value to all other listeners for this attribute
    if self._handlers.has_key(attr):
      [listener(self[attr]) for listener in self._handlers[attr]]
...
def normalize_python(node=None, dom=None, buf=None, prefix=None, definition=False):
  ...
  elif node:  value = ''.join([x.nodeValue for x in node._elem.childNodes])
  else:       value = buf
...
import ViewProjector, Events, InitGL, Basetags
