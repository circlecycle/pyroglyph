...
import Canvas, Events, Classes, Helpers, Cocoa
import sys, os, weakref, time, pyglet

class _class(Canvas.Node):
  """
    for each class tag encoutered, assemble and execute one class definition in Classes,
  ...
  def __construct__(self):
    name = self.vbl_elem.getAttribute('name')
    Canvas.CLASSES_ACCUMULATOR[name] = self
    extends = self.vbl_elem.getAttribute('extends') or "view"
    definition = "class %s(%s):\n  def __construct__(self): %s.__construct__(self)"%(name, extends, extends)
    exec definition in globals()
    self.delete()

class node(Canvas.Node):
  """ 
...
class view(Canvas.Node):
  def __construct__(self):
    ns = self.__dict__    
    
    #set the display type of the node to view
    ns['vbl_displaytype'] = 'view'
    ...
    if not self.vbl_elem.getAttribute('width'): ns['_nowidth'] = True
    if not self.vbl_elem.getAttribute('height'): ns['_noheight'] = True
      
    ####### APPLY DEFAULTS #################### # # #  #   #    #
    
    #(which doesn't override - but typecasts! - preexisting string attrs on the element)
    self.applydefaults( { 'width':0, 'height':0, 'image':'', 'aligned':'', 'rotate':0.0,
    ...
    elif 'auto' in self.sizing and self.vbl_classname != 'import':
      if self['_nowidth']: 
    ...
    if 'auto' in self.parent.sizing and 'skip' not in self.sizing and self.parent.vbl_classname != 'import':
    ...
    if self.vbl_after and self.vbl_after in self.parent.childViews:
      self.parent.childViews.insert(self.parent.childViews.index(self.vbl_after), self)
    else:
      self.parent.childViews.append(self)
      
    self.__dict__['vbl_cocoaview'] = Cocoa.View(node=self, parent=self.parent)
  ...
  def _init(self, val=False):
    ...
    self.vbl_cocoaview.setNeedsDisplay_(True)
...
class text(view):
  ...
  def __construct__(self):
    ...
    self.__dict__['vbl_displaytype'] = 'text'
  ...
  def _textSize(self, val=False):    
    ...
    return ## TODO ENABLE AFTER COOCA SUPPORT
...
class _import(view):
  ...
  def __construct__(self):
    ...
    else:     self.__dict__['_includePath'] = "%s/%s"%(Helpers.baselibs, self.src)
    ...
    modsToLoad = [( x.strip(), Canvas._import(x.strip()) ) 
                    for x in domToLoad.getAttribute('import').split(',') 
                      if x and x not in Canvas.GLOBALS_ACCUMULATOR]
    ...
    [Canvas.GLOBALS_ACCUMULATOR.append(x) for x in modsToLoad]
...
class font(Canvas.Node):
  ...
  def __construct__(self):
    ...
    return ## TODO: restore font stuff
...
class replicate(view):
  ...
  def update(self, val=None):
    ...
    for entry in self.over:
      for elem in self.vbl_elem.childNodes:
...
class dataset(Canvas.Node):
  ...
  def __construct__(self):
    ...
    if(self.url and self.auto): 
      ...
      self.childNodes = [x for x in self.vbl_elem.childNodes if x.nodeType == x.ELEMENT_NODE]
      self.touch('data')
